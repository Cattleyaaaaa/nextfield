...
export function PageTransitionProvider({
  ...
  const [motionSetting, setMotionSetting] = useState<"system" | "on" | "off">(
    "system",
  );
  const [phase, setPhase] = useState<"idle" | "covering" | "revealing">("idle");
  const targetRef = useRef<string | null>(null);
  const motionEnabled =
    motionSetting === "on" || (motionSetting === "system" && !reducedMotion);

  useEffect(() => {
    const savedSetting = window.localStorage.getItem("water-motion-setting");
    if (savedSetting === "on" || savedSetting === "off")
      setMotionSetting(savedSetting);
  }, []);
  ...
  const navigate = useCallback(
    (href: string, origin?: TransitionOrigin) => {
      const samePage = comparablePath(href) === comparablePath(pathname);
      if (samePage || targetRef.current || !motionEnabled) {
  ...
  useEffect(() => {
    const targetPathname = targetRef.current;
    if (
      phase === "covering" &&
      targetPathname &&
      comparablePath(targetPathname) === comparablePath(pathname)
    ) {
      requestAnimationFrame(() => setPhase("revealing"));
    }
  }, [pathname, phase]);

  // A failed or unusually slow route must never leave an opaque curtain over the site.
  useEffect(() => {
    if (phase !== "covering") return;
    const timeout = window.setTimeout(() => setPhase("revealing"), 4000);
      ...
      {motionEnabled && (
        ...
        >
          <motion.div
            ...
            transition={{
              duration: phase === "covering" ? 0.48 : 0.56,
              ease: [0.65, 0, 0.25, 1],
            }}
          ...
          <motion.div
            ...
            transition={{
              duration: phase === "covering" ? 0.46 : 0.58,
