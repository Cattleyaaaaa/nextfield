...
export function LessonExperience({ track, lesson, previous, next }: { track: LearningTrack; lesson: LearningLesson; previous?: LearningLesson; next?: LearningLesson }) {
  ...
  return <article className="mx-auto max-w-site px-5 pb-28 pt-16 sm:px-8 sm:pt-24 lg:px-12">
    ...
    <header className="mt-12 max-w-5xl"><div className="flex flex-wrap items-center gap-3 font-mono text-[10px] uppercase tracking-[0.16em] text-accent"><span>PATH {track.number}</span><span className="text-line">/</span><span>LESSON {lesson.number}</span><span className="text-line">/</span><span>{lesson.minutes} {locale === "zh" ? "分钟" : "min"}</span>{done ? <span className="inline-flex items-center gap-1.5 rounded-full bg-accent/10 px-2 py-1"><CheckCircle2 className="size-3" />{locale === "zh" ? "已完成" : "Complete"}</span> : null}</div><h1 className="mt-6 text-balance font-display text-[clamp(3.25rem,8vw,7rem)] leading-[0.88] tracking-[-0.065em]">{lesson.title[locale]}</h1><p className="mt-8 max-w-3xl text-lg leading-9 text-muted">{lesson.summary[locale]}</p></header>

    <section className="mt-16 grid gap-10 border-t border-line pt-12 lg:grid-cols-[12rem_minmax(0,48rem)] lg:gap-16"><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-accent">01 / {locale === "zh" ? "核心概念" : "Concept"}</p><div className="space-y-6">{lesson.concept.map((paragraph) => <p className="text-base leading-8 text-muted sm:text-lg sm:leading-9" key={paragraph.en}>{paragraph[locale]}</p>)}</div></section>

    <section className="mt-16 grid gap-10 border-t border-line pt-12 lg:grid-cols-[12rem_minmax(0,1fr)] lg:gap-16"><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-accent">02 / {locale === "zh" ? "心智模型" : "Mental model"}</p><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{lesson.model.map((item, index) => <article className="rounded-2xl border border-line bg-panel p-5" key={item.label.en}><span className="font-mono text-[9px] text-accent">{String(index + 1).padStart(2, "0")}</span><h2 className="mt-10 font-display text-2xl">{item.label[locale]}</h2><p className="mt-2 text-xs leading-5 text-muted">{item.detail[locale]}</p></article>)}</div></section>

    <section className="mt-16 grid gap-10 border-t border-line pt-12 lg:grid-cols-[12rem_minmax(0,48rem)] lg:gap-16"><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-accent">03 / {locale === "zh" ? "代码骨架" : "Code skeleton"}</p><div className="overflow-hidden rounded-2xl bg-ink text-paper"><div className="flex items-center justify-between border-b border-paper/10 px-5 py-3"><span className="font-mono text-[9px] tracking-[0.14em] text-paper/45">TYPESCRIPT / CONCEPT</span><button className="inline-flex items-center gap-2 text-xs text-paper/55 hover:text-liquid-foam" onClick={copyCode} type="button">{copied ? <Check className="size-3.5" /> : <Clipboard className="size-3.5" />}{copied ? (locale === "zh" ? "已复制" : "Copied") : (locale === "zh" ? "复制" : "Copy")}</button></div><pre className="overflow-x-auto p-5 text-sm leading-7 text-liquid-foam"><code>{lesson.code}</code></pre></div></section>

    <section className="mt-16 grid gap-10 border-t border-line pt-12 lg:grid-cols-[12rem_minmax(0,48rem)] lg:gap-16"><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-accent">04 / {locale === "zh" ? "检查点" : "Checkpoint"}</p><div className="rounded-[2rem] border border-line bg-panel p-6 sm:p-8"><h2 className="font-display text-3xl leading-tight tracking-[-0.04em]">{lesson.challenge.question[locale]}</h2><div className="mt-8 space-y-3">{lesson.challenge.options.map((option, index) => <button className={`w-full rounded-2xl border p-4 text-left text-sm transition-colors ${choice === index ? option.correct ? "border-accent bg-accent/[0.08]" : "border-red-400/60 bg-red-400/[0.06]" : "border-line bg-paper hover:border-accent"}`} key={option.label.en} onClick={() => setChoice(index)} type="button">{option.label[locale]}</button>)}</div>{choice !== null ? <div className="mt-6 border-t border-line pt-5"><p className="text-sm leading-7 text-muted">{lesson.challenge.options[choice].feedback[locale]}</p><div className="mt-5 flex flex-wrap gap-3">{correct && !done ? <button className="inline-flex items-center gap-2 rounded-full bg-ink px-5 py-3 text-sm text-paper" onClick={() => complete(key)} type="button"><Check className="size-4" />{locale === "zh" ? "完成本课" : "Complete lesson"}</button> : null}{!correct ? <button className="inline-flex items-center gap-2 text-xs text-muted hover:text-accent" onClick={() => setChoice(null)} type="button"><RotateCcw className="size-3.5" />{locale === "zh" ? "重新选择" : "Try again"}</button> : null}</div></div> : null}</div></section>
