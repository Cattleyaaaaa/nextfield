...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // The about page remains usable when WebGL is unavailable.
      return;
    }
    ...
    function render(target: FBO | null) {
      if (!target && config.TRANSPARENT) {
        // The official TS variant exposes TRANSPARENT but leaves the default
        // framebuffer opaque. Clear its alpha before the display pass.
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    ...
    return () => {
      ...
      // React dev mode replays effects on the same connected canvas. Release
      // only after the DOM node is actually removed by a route/theme change.
      window.setTimeout(() => {
