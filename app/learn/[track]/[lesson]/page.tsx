...
export default async function LearningLessonPage({ params }: { params: Promise<{ track: string; lesson: string }> }) { const { track: trackSlug, lesson: lessonSlug } = await params; const track = findLearningTrack(trackSlug); const lesson = track ? findLearningLesson(track, lessonSlug) : undefined; if (!track || !lesson) notFound(); const index = track.lessons.indexOf(lesson); return <LessonExperience guide={LEARNING_GUIDES[`${track.slug}/${lesson.slug}`]} lesson={lesson} next={track.lessons[index + 1]} previous={track.lessons[index - 1]} track={track} />; }
