...
export function RadioDisc({
  ...
  return (
    ...
    <div
      aria-hidden="true"
      className={`relative shrink-0 ${className ?? "size-20"}`}
    >
      <div
        ...
        style={{
          animationPlayState: playing && motionEnabled ? "running" : "paused",
        }}
      >
        {/* 纹路：细密同心圆，转起来才看得出在动 */}
        ...
        <span className="absolute inset-[22%] overflow-hidden rounded-full">
          {showCover ? (
            // eslint-disable-next-line @next/next/no-img-element -- Supports local artwork and external archive URLs without image optimization.
            <img
              alt=""
            ...
            <span
              ...
              style={{
                backgroundImage: `linear-gradient(140deg, hsl(${hue} 62% 54%), hsl(${(hue + 52) % 360} 58% 30%))`,
              }}
            />
          )}
        </span>
