...
import { useRef, useEffect, useSyncExternalStore, type CSSProperties, type ReactNode, type MouseEventHandler, type RefObject } from 'react';
...
const SpecularButton = ({
  ...
  const finePointer = useSyncExternalStore(subscribeToPointer, hasFinePointer, () => false);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Keep the native control usable when WebGL is unavailable.
      return;
    }
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (proximityT > 0 || bright > 0.002) requestRender();
    };
    ...
    const update = (now: number) => {
      ...
      const needsFrame = p.autoAnimate || Math.abs(brightTarget - bright) > 0.002 || (brightTarget > 0.002 && Math.abs(diff) > 0.002);
      if (needsFrame) requestRender();
    };
    requestRender = () => {
