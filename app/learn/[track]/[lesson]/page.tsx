...
export default function LearningLessonPage({ params }: { params: { track: string; lesson: string } }) { const track = findLearningTrack(params.track); const lesson = track ? findLearningLesson(track, params.lesson) : undefined; if (!track || !lesson) notFound(); const index = track.lessons.indexOf(lesson); return <LessonExperience lesson={lesson} next={track.lessons[index + 1]} previous={track.lessons[index - 1]} track={track} />; }
