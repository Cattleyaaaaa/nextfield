...
import { ExamCallout } from "./exam-callout";
...
function downloadPass(completedTracks: string[]) {
  const labels = LEARNING_TRACKS.filter((track) =>
    completedTracks.includes(track.slug),
  )
    .map((track) => track.title.en)
    .join(" · ");
  ...
  anchor.href = url;
  anchor.download = "NEXTFIELD-Field-School-Pass.svg";
  anchor.click();
...
export function LearningHub() {
  ...
  const completedTracks = LEARNING_TRACKS.filter((track) =>
    track.lessons.every((lesson) =>
      completed.includes(lessonKey(track.slug, lesson.slug)),
    ),
  ).map((track) => track.slug);
  const total = LEARNING_TRACKS.reduce(
    ...
    <div>
      ...
      <section id="curriculum" className="scroll-mt-28">
        <h2 className="mb-5 font-display text-3xl">
          {locale === "zh" ? "选择你的起点" : "Choose your starting point"}
        </h2>
        <div className="grid gap-4 lg:grid-cols-3">
          {LEARNING_TRACKS.map((track) => {
            const count = track.lessons.filter((lesson) =>
              completed.includes(lessonKey(track.slug, lesson.slug)),
            ).length;
            return (
              <TransitionLink
                className="group flex min-h-[18rem] flex-col rounded-[2rem] border border-line bg-panel p-6 hover:border-accent sm:p-8"
              ...
              >
                ...
                <div className="mt-8">
                  <p className="mb-4 text-xs text-accent">
                    ...
                    {locale === "zh"
                      ? "节课 · 4 个阶段 · 项目 + 考试"
                      : "lessons · 4 stages · project + exam"}
                  </p>
                  <h2 className="font-display text-4xl tracking-[-0.05em]">
                    {track.title[locale]}
                  </h2>
                  ...
                  <div className="mt-8 flex items-center gap-3">
                    <span className="h-1 flex-1 overflow-hidden rounded-full bg-line">
                      <span
                        ...
                        style={{
                          width: `${(count / track.lessons.length) * 100}%`,
                        }}
                      />
                    </span>
      ...
      <ExamCallout />
      <LearningResources />
      <section className="mt-6 grid gap-8 rounded-[2rem] bg-ink p-7 text-paper sm:p-10 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <div className="flex items-center gap-3">
            <Award className="size-5 text-liquid-foam" />
            <p className="font-mono text-[10px] tracking-[0.16em] text-liquid-foam">
              FIELD PASS
            </p>
          </div>
          <h2 className="mt-8 font-display text-4xl tracking-[-0.05em]">
            {locale === "zh"
              ? "把完成的路径带走。"
              : "Take your completed path with you."}
          </h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-paper/55">
            {locale === "zh"
              ? `当前完成 ${completed.length}/${total} 节。完成任意一条路径后，可在本地生成学习凭证；访客记录不上传；登录后使用云端进度。此凭证是自报进度纪念卡，不是服务器认证。`
              : `${completed.length}/${total} lessons complete. Finish any path to generate a local learning pass—guest progress stays local; signed-in progress syncs. This pass is a self-reported keepsake, not server certification.`}
          </p>
        </div>
        <button
          className="inline-flex items-center justify-center gap-2 rounded-full bg-liquid-foam px-5 py-3 text-sm text-ink disabled:cursor-not-allowed disabled:opacity-35"
          disabled={completedTracks.length === 0}
          onClick={() => downloadPass(completedTracks)}
          type="button"
        >
          ...
          {locale === "zh" ? "生成 Field Pass" : "Generate Field Pass"}
        </button>
      </section>
    </div>
  );
